/*
This program, when run, brings up a GUI to control the DriveBot.
Please make sure that the Serial Communication is successfully established between the robot and the PC before running this program.
Also, please change the COM Port and Baudrate as required.
The movements of the robot can be controlled using Arrow Keys on the keyboard.
To stop the robot, simply press the Space Bar on the keyboard.
*/
...
String T= "DriveBot Controller GUI"; //Creating word strings for the interface.
...
void setup()
{
  ...
  myPort = new Serial(this, "COM#", 9600); //Initializing the serial port with 9600 baud rate [Specify the required COM Port & baud rate for serial communication].
}
